...
> **Before you start:** while authoring, you'll spend most of your time in the
> **GDQuest** tab in the editor's right-hand dock (next to Inspector/Node/History) —
> that's where the practices list, Reset button, and live test panel live. See §5
> for the full tour. If you're new to this addon, it's also worth skimming the
> gotchas in §3 (comment-stub syntax) and §4 (`_setup_state`/timing) before writing
> your first `test.gd` — they cover the mistakes that are easiest to make and
> hardest to debug from the symptom alone.
...
**Reach for this only when a change can't be expressed as a script-line
substitution.** Most "make the practice's starting point different from the
solution" needs — changing an export default, an initial value, anything that's
just a literal on a line — are better handled by the comment-stub syntax above:
...
`L1.P3.visual_velocity_comparison` (§9) is a worked example of exactly this — an
earlier version of that practice used a `diff.gd` purely to zero out an export
default, and switching to the comment-stub line above made the `diff.gd` file
unnecessary entirely. Reserve `diff.gd` for genuinely **structural** changes to the
scene tree that no script edit could produce — removing a node the student is meant
to add, or changing a node's type.

If you do need it: `practice_solutions/<dir>/diff.gd` can define a static function
whose name exactly matches a `.tscn` file's basename (e.g. a function named
`practice` for `practice.tscn`). `build.gd` instantiates the **solution** scene,
calls that function on it, and packs the *mutated* result as the practice scene:
