# Godot Engine documentation PreTeXt version

This README will describe how to convert the ReStructured Text into PreTeXt so that it can be converted into a Runestone.academy textbook.

## Conversion from RestructuredText to Pretext Steps



   1. In the parent directory make a virtual environment and install sphinx and pretext as follows:
        ...
        pip install sphinx_rtd_theme sphinx-copybutton sphinxcontrib-video
        pip install sphinx-notfound-page
        pip install sphinxext-opengraph
        
   2. Then, still in the parent directory type the following

        python -m sphinx -b xml .  _build/xml
        python sphinx_xml_to_pretext.py \
              _build/xml pretext/source \
              --index-doc index \
              --title "Godot Docs (Converted)" \
              --env-pickle _build/xml/.doctrees/environment.pickle
        python fix_pretext_images.py --ptx-dir pretext/source --html-dir . --xml-dir _build/xml 
        
  3. Then cd into the `pretext` directory and type the following
